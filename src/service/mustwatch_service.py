...
class MustwatchService:
    ...
    def __get_watches_dict_for_add_to_one_user(self,
        ...
        chosen_user_watches_id_list = self.mustwatches_read.get_watches_id_list_for_add_to_one_chosen_user(
            chosen_user_tuple)
        watches_id_list = self.mustwatches_read.get_watches_id_list_for_add_to_one_user(same_group_users_id_list,
    ...
    def __get_watches_dict_for_rating_mustwatch(self,
                                                chosen_user_tuple: Tuple[int]) -> Dict[int, str]:
        watches_id_list = self.mustwatches_read.get_watches_id_list_for_rating_mustwatch(chosen_user_tuple)
        return self.__make_watches_dict_from_watches_id_list(watches_id_list)
    ...
    def __add_mustwatch_to_tables(self,
                                  ...
                                  chat_id: str) -> None:
        watch_record = self.watches_read.get_record_by_title_and_chat_id(title, chat_id)
        if not watch_record:
            self.watches_redact.insert_values(title=title, group_id=chat_id)
            watch_record = self.watches_read.get_record_by_title_and_chat_id(title, chat_id)
        ...
        for users_id in chosen_user_id:
            if users_id not in users_with_chosen_mustwatch:
                self.mustwatches_redact.insert_values(watches_id=watch_record.id, users_id=users_id)
    ...
    def __get_user_request(self,
                           chat_id: str,
                           user_id: str) -> Query:
        user = self.users_read.get_record(chat_id, user_id)
        return self.user_requests_read.get_record(user)
    ...
    def update_user_request_chosen_title(self,
        ...
        watch = self.watches_read.get_record_by_id(chosen_title_id)
        try:
            user = self.users_read.get_record(chat_id, user_id)
    ...
    def get_chosen_user_from_user_request(self,
        ...
        if chosen_user_id.isdigit():
            ...
            user = self.users_read.get_record_by_id(chosen_user_id)
